...
// Helper function to get Redis store or undefined for fallback
const getRedisStore = (prefix: string): any => {
  if (redis && isRedisAvailable) {
    try {
      return new RedisStore({
        // @ts-ignore - rate-limit-redis types may be outdated
        sendCommand: (...args: string[]) => redis.call(...args),
        prefix,
      });
    } catch (err) {
      console.error('Error creating RedisStore:', err);
      return undefined;
    }
  }
  return undefined;
